...
(function () {
  ...
  function init() {
    ...
    /* ---------- Opportunities list view (Week 2) ---------- */
    var opApp = document.getElementById("screen-opportunities");
    if (opApp && opApp.children.length > 0) {
      /* Approved inline notice for all undefined modals/toolbar/list/tab controls. */
      var opToast = document.getElementById("op-toast");
      var opToastTimer = null;
      function opShowNotice() {
        if (!opToast) return;
        opToast.textContent = "Not part of this prototype.";
        opToast.hidden = false;
        if (opToastTimer) clearTimeout(opToastTimer);
        opToastTimer = setTimeout(function () { opToast.hidden = true; }, 2200);
      }
      var opNoticeEls = opApp.querySelectorAll(".hd-notice");
      for (var o = 0; o < opNoticeEls.length; o++) {
        opNoticeEls[o].addEventListener("click", function (e) {
          e.preventDefault();
          opShowNotice();
        });
      }
      ...
      /* Profile button toggles a small menu containing Logout only. */
      var opProfileBtn = document.getElementById("op-profile-btn");
      var opProfileMenu = document.getElementById("op-profile-menu");
      if (opProfileBtn && opProfileMenu) {
        opProfileBtn.addEventListener("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          opProfileMenu.hidden = !opProfileMenu.hidden;
        });
        document.addEventListener("click", function () {
          if (!opProfileMenu.hidden) opProfileMenu.hidden = true;
        });
      }

      /* Logout (SCREEN_MAP) -> clear fake auth + route to login-main. */
      var opLogout = document.getElementById("op-logout");
      if (opLogout) {
        opLogout.addEventListener("click", function (e) {
          e.preventDefault();
          localStorage.removeItem(LS.AUTH);
          Router.navigate("login-main");
        });
      ...
      /* Functional list search (Week 2). Defensive: existence-guarded, filters
         only .op-tbody tr rows by textContent (never injects input into the DOM),
         empty query shows all, toggles the hidden empty state, no-throw on 0 rows,
         scoped to this screen only. */
      var opSearch = opApp.querySelector(".op-search");
      var opTbody = opApp.querySelector(".op-tbody");
      var opEmpty = document.getElementById("op-empty");
      /* The empty state reuses .ac-empty (display:flex), which overrides the
         [hidden] attribute, so control visibility via inline display instead. */
      if (opEmpty) opEmpty.style.display = "none";
      if (opSearch && opTbody) {
        opSearch.addEventListener("input", function () {
          var q = this.value.trim().toLowerCase();
          var rows = opTbody.querySelectorAll("tr");
          var visible = 0;
          ...
          if (opEmpty) opEmpty.style.display = (visible === 0) ? "flex" : "none";
        });
      }
    }
